# This Makefile is for the CMS::MediaWiki extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/CMS/MediaWiki.pm]
#     AUTHOR => q[Reto Hersiczky <retoh@localdomain>]
#     DEFINE => q[]
#     INC => q[-I.]
#     LIBS => [q[]]
#     NAME => q[CMS::MediaWiki]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[lib/CMS/MediaWiki.pm]
...
FULLEXT = CMS/MediaWiki
BASEEXT = MediaWiki
PARENT_NAME = CMS
DLBASE = $(BASEEXT)
VERSION_FROM = lib/CMS/MediaWiki.pm
INC = -I.
DEFINE = 
...
XS_FILES = MediaWiki.xs
C_FILES  = MediaWiki.c
O_FILES  = MediaWiki.o
H_FILES  = ppport.h
MAN1PODS = 
MAN3PODS = lib/CMS/MediaWiki.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DIRFILESEP)Config.pm $(PERL_INC)$(DIRFILESEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/CMS
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/CMS
...
TO_INST_PM = lib/CMS/MediaWiki.pm

PM_TO_BLIB = lib/CMS/MediaWiki.pm \
...
DISTNAME = CMS-MediaWiki
DISTVNAME = CMS-MediaWiki-0.80.03
...
# CMS::MediaWiki might depend on some other libraries:
...
manifypods : pure_all  \
	lib/CMS/MediaWiki.pm \
	lib/CMS/MediaWiki.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  lib/CMS/MediaWiki.pm $(INST_MAN3DIR)/CMS::MediaWiki.$(MAN3EXT) 
...
clean :: clean_subdirs
	-$(RM_RF) MediaWiki.c ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all $(INST_ARCHAUTODIR)/extralibs.ld perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core core.[0-9] core.[0-9][0-9] core.[0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9] core.[0-9][0-9][0-9][0-9][0-9]
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_F) $(INST_STATIC)
	$(RM_F)  blib/lib/CMS/MediaWiki.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         CMS-MediaWiki' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.80.03' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: lib/CMS/MediaWiki.pm' >> META.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,80,03,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension for managing MediaWiki pages</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Reto Hersiczky &lt;retoh@localdomain&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/CMS/MediaWiki.pm blib/lib/CMS/MediaWiki.pm 
	$(NOECHO) $(TOUCH) $@
